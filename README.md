...
BGM と効果音も付いています。**どちらも既定でオン**（BGM は効果音より控えめな音量から始まります）。ヘッダーのスピーカーボタンからミュートと 3 系統の音量を調整でき、設定は次回以降も保持されます。
...
公開サイト（`hummer98.dev`）では、自前のコレクタ（Cloudflare Workers + D1、[`tools/collector/`](tools/collector/)）で最小限の計測を行っています。clone した状態では `prototype/analytics.js` の `endpointUrl` を書き換えるか `provider` を `"none"` に戻せば、**何も送信しません**。

収集する内容は次の条件を満たすものだけです。

- Cookie / 端末識別子を使わない
- 送るのはページビューと「どこまで進んだか」だけ。入力内容や解答は送らない
- **IP アドレスは保存しない。** User-Agent も `mobile` / `desktop` の 2 値にだけ落とす
